    ...
    for cv_type in cv_types:
        for n_components in n_components_range:
            ...
            if bic[-1] < lowest_bic:
                lowest_bic = bic[-1]
                best_gmm1 = gmm
    with open('gmm_model_sift.pkl','wb') as ffid:
        cPickle.dump(best_gmm1, ffid)
    final = []
    for i in range(K):
        fv = fisher_vector(X[i], best_gmm)
        fv1 = fisher_vector(X1[i], best_gmm1)
        fv = np.array(fv)
        fv = fv.transpose()
        fv1 = np.array(fv1)
        fv2 = fv.dot(fv1)
        fv2.reshape(1,fv2.shape[0]*fv2.shape[1])
        final.append(fv2)
    final = np.array(final)
    clf = RandomForestClassifier(max_depth=5, random_state=0)
    clf.fit(final,Y)
    with open('cnn_sift.pkl', 'wb') as fid:
